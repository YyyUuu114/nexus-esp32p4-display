...
本分支为固件开发分支，当前版本为 **2.1.1**，不作为正式部署基线。正式固件位于 [`firmware-stable`](https://github.com/YyyUuu114/nexus-esp32p4-display/tree/firmware-stable)，对应的 Windows 开发分支位于 [`desktop-dev`](https://github.com/YyyUuu114/nexus-esp32p4-display/tree/desktop-dev)。

v2.1.1 引入双向设备握手、运行时兼容范围校验和会话 nonce，线协议升级为 2.0，更新分类为 `coordinated-breaking`。必须同时使用桌面端 v2.1.1；v1.x 与 v2.x 混用会被明确拒绝。

## 运行特性

- 1 Hz 主机遥测刷新，曲线保留最近 60 个样本。
- 3.5 秒无合法数据进入 `WAIT`；15 秒无活动后关闭背光并停止 LVGL、触摸和 MIPI-DSI，握手后自动恢复。
...
- 桌面端通过随机 nonce 双向握手识别设备，不会把“唯一可打开串口”误判为 NEXUS。
...
Windows 的 ESP-IDF 依赖路径较深；若工程位于较长目录并出现路径长度错误，请在已初始化的 ESP-IDF 终端运行 `./tools/build-windows.ps1`。脚本临时映射短盘符并使用独立的 `build-short` 目录完成构建，退出时解除映射，不修改系统长路径策略。打包该输出时向 `package-release.ps1` 传入 `-BuildDirectory build-short`。
...
完整判定规则见 [COMPATIBILITY.md](COMPATIBILITY.md)，机器可读规则见 [compatibility.json](compatibility.json)，当前发布元数据见 [release.json](release.json)。v2.1.1 是协议 2.0 的开发配对基线，必须同时升级固件和桌面端；正式 v1.1.0 基线保持不变。
